...
const ProjectMain = ({projectInfo, isTemplate}) => {
    ...
    const gotoEdit = () => {
        if (isTemplate) {
            navigate(`${lang === undefined ? '/' : '/' + lang + '/'}profile/${username}/template-edit/${template}`)
        } else {
            navigate(`${lang === undefined ? '/' : '/' + lang + '/'}profile/${username}/project-edit/${project}`)
        }
    }
    ...
    return (
        <>
            ...
            <div className="project__last-info">
                <div>&gt; <span><Link
                    to="/">{projectInfo.info.username}</Link></span>, {projectInfo.info.role === 'admin' ? "Администрация" : "Пользователь"}, {projectInfo.info.date_of_publication}
                </div>
