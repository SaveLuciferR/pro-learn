...
const CourseBlockItem = ({ obj }) => {
    ...
    const getStatus = (item) => {
        switch (item.success) {
            case 0: {
                return (
                    <div className="course-blocks-item-lessons-item-status blocked">
    ...
    const getAmountCompletedLesson = () => {
        let amount = 0;
        obj['step_course'].map((item, i) => {
            if (item.success === 1) {
                amount += 1;
            }
        })
    ...
    const getAmountLesson = () => {
        return obj['step_course'].length;
    }
        ...
        <div className="course-blocks-item">
            <div className="course-blocks-item-header">
                <div className="course-blocks-item-header-l">
                    <p className="course-blocks-item-header-title">_{obj.title}</p>

                    {getStatus(obj)}

                    <p className="course-blocks-item-header-progress">{getAmountCompletedLesson()}/{getAmountLesson()}</p>
                </div>
                <svg
                    width="36"
                    height="36"
            ...
            {activeList ? (
                <ul className="course-blocks-item-lessons">
                    {obj['step_course'].map((item, i) =>
                        <li key={i} className="course-blocks-item-lessons-item">
                            <Link to={`study?block=${obj.num_stage}&lesson=${item.num_step}`}>&gt; {item.title}</Link>
                            {getStatus(item)}
                        </li>
                    )}
                </ul>
