...
const ProfileSettingsPrivacy = () => {
    ...
    const [allProfilePrivate, setAllProfilePrivate] = useState('');
    const [lookCurrentCoursePrivate, setLookCurrentCoursePrivate] = useState('');
    const [personalInfoPrivate, setPersonalInfoPrivate] = useState('');

    useEffect(() => {
        axiosClient.get(`${lang === undefined ? '/' : '/' + lang + '/'}@${username}/settings/privacy`)
            .then((res) => {
                if (res.data.profile_privacy.success) {
                    setAllProfilePrivate(res.data.profile_privacy.privacy.all_profile_private);
                    setLookCurrentCoursePrivate(res.data.profile_privacy.privacy.look_current_course_private);
                    setPersonalInfoPrivate(res.data.profile_privacy.privacy.personal_info_private);
                }
    ...
    useEffect(() => {
        if (canBeUpdate) {
            axiosClient.post(`${lang === undefined ? '/' : '/' + lang + '/'}@${username}/settings/privacy`, {
                all_profile_private: allProfilePrivate,
                look_current_course_private: lookCurrentCoursePrivate,
    ...
    }, [lookCurrentCoursePrivate, allProfilePrivate, personalInfoPrivate])
    ...
    return (
        <div>
            <div className="created-courses-header">
                ...
                <h1>_{viewWords['tpl_profile-privacy_title']}</h1>
            </div>
            <div className="profile-settings-privacy">
                <div className="profile-settings-privacy-main">
                    <div className="profile-settings-privacy-main_item">
                        ...
                        <button
                            onClick={() => setAllProfilePrivate(prevState => prevState === '1' ? '0' : '1')}
                            className="btn big primary width100p">
                            {viewWords[`tpl_profile-privacy_all-profile-${allProfilePrivate}`]}
                        </button>
                        <p className="profile-settings-text small opacity60">
                            {viewWords['tpl_profile-privacy_all-profile-desc']}
                        </p>
                    </div>
                    <div className="profile-settings-privacy-main_item">
                        <svg
                        ...
                        <button
                            onClick={() => setLookCurrentCoursePrivate(prevState => prevState === '1' ? '0' : '1')}
                            className="btn big primary width100p">
                            {viewWords[`tpl_profile-privacy_current-course-${lookCurrentCoursePrivate}`]}
                        </button>
                        <p className="profile-settings-text small opacity60">
                            {viewWords['tpl_profile-privacy_current-course-desc']}
                        </p>
                    </div>
                    <div className="profile-settings-privacy-main_item">
                        <svg
                        ...
                        <button
                            onClick={() => setPersonalInfoPrivate(prevState => prevState === '1' ? '0' : '1')}
                            className="btn big primary width100p">
                            {viewWords[`tpl_profile-privacy_personal-info-${personalInfoPrivate}`]}
                        </button>
                        <p className="profile-settings-text small opacity60">
                            {viewWords['tpl_profile-privacy_personal-info-desc']}
                        </p>
                    </div>
                </div>
            </div>
