...
const SidebarProfile = () => {
    ...
    const onClickLogout = () => {
        axiosClient.post('/user/logout', {client: localStorage.getItem('client')})
            .then(({data}) => {
                dispatch(setSidebarProfileActive(false));
                navigate(currentUrl);
    ...
    return (
        <div
            className={`filter${sidebarProfileActive ? ' active' : ''}`}>{/* Тернарный оператор на раздачу активного класса */}
            <div className="filter_container">
                {

                    currentUser.length === 0 ?
                        <div className="sidebar_profile-empty">
                            <button onClick={() => onClickLogin()} type="button" className="btn big primary ">Авторизация</button>
                            <button onClick={() => onClickRegister()} type="button" className="btn big primary ">Регистрация</button>
                        </div>
                        :
                        <>
                            ...
                            <div className="sidebar_profile-body">
                                ...
                                <button type="button" className="btn primary big">Перейти в профиль</button>
                                <button onClick={() => onClickLogout()} type="button" className="btn">Выйти</button>
