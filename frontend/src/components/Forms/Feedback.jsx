...
const Feedback = () => {
    ...
    async function OnClickFeedback() {
        ...
        if (name.length !== 0 && email.length !== 0 && message.length !== 0) {
            await axiosClient.post("/user/save-feedback", {name, email, currentFeedbackCategory, message})
                .then(({data}) => {
    ...
    useEffect(() => {
        axiosClient.get(`${lang === undefined ? "/" : '/' + lang + "/"}user/feedback-category`)
            .then(({data}) => {
    ...
    return (
        <>
                ...
                <form className="modal_form">
                    ...
                    <div className="form_input">
                        <div className="form_input-item">
                            <input placeholder=" " type="text" name="name" id="name" required="required" value={name}
                                   onChange={(e) => setName(e.target.value)}/>
                            <span>Имя</span>
                        </div>
                        <div className="form_input-item">
                            <input placeholder=" " type="email" name="email" id="email" required="required"
                                   value={email}
                            ...
                            <ul className={`dropdown-menu  ${dropdownMenuActive ? "active" : ""}`}>
                                {feedbackCategories.map((item) =>
                                    item.code === currentFeedbackCategory.code ? <></> :
                                        <li key={item.id} id={item.id} data-id={item.id} onClick={() => setCurrentFeedbackCategory(item)}>{item.title}</li>
                                )}
                            </ul>
                        </div>
                        <div className="form_input-item">
          <textarea className="input textarea scroll" placeholder="Сообщение" value={message}
                    ...
                    <button className="btn big primary" type="button" onClick={() => OnClickFeedback()}>Отправить
                    </button>
