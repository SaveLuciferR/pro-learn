...
import {
    ...
    setCompilerCurrentFileName, setCurrentFolder,
...
const CompilerSidebarList = ({folder, compilerFiles, tab, localPath, showNav}) => {
    ...
    return (
        <>
            ...
            >
                {isOpenedFolder ? <FaFolderOpen className={"icon-24"}/> : <FaFolder className={"icon-24"}/>}
                {folder}
            </div>
            <Collapse isOpened={isOpenedFolder}>
                <div className="compiler__file" style={{paddingLeft: `${tab}px`}} data-project-element={localPath + '/' + folder}>
                    {getFilesStructure}
                </div>
            </Collapse>
        </>
