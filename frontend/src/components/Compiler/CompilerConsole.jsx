...
const CompilerConsole = ({
  ...
  useEffect(() => {
    ...
    const text = output === undefined ? '' : output.split('\n');
    text.map((item) => {
  ...
  const handleClickTask = (key) => {
    if (!taskIsLoading) {
      ...
      axiosClient
        .get(`/compiler/@${username}/${project}/start-task&task=${key}`)
        .then((res) => {
  ...
  return (
    <div className="console">
      ...
      <div className="console-text scroll">
        <div className="console-text-div">
          {terminalContent.map((item) => (
            <p className={'console-text-item'}>{item}</p>
          ))}
      ...
      <CompilerContextMenuTasks
        context={context}
        setContext={setContext}
        xyPos={positionContext}
        tasks={tasks}
