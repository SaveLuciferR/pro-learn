...
const CompilerPage = ({ isSolve, isActiveSidebar, isCompiler }) => {
  ...
  const sendRequestTerminal = (req) => {
    ...
    startDockerContainer(
      `http://api.pro-learn.my/compiler/@${username}/${project}/request-terminal?req=${req}`,
    );
  ...
  const solvedTask = () => {
    ...
    axiosClient
      .post(`/compiler/@${username}/${project}/${task}/check-solution-task`, [])
      .then(({ data }) => {
  ...
  useEffect(() => {
    if (updateFiles) {
      axiosClient
        .post(`/compiler/@${username}/${project}`)
        .then(({ data }) => {
          console.log(data);
  ...
  useEffect(() => {
    if (task !== undefined) {
      axiosClient
        .get(`${lang === undefined ? '/' : '/' + lang + '/'}compiler/solve-task?task=${task}`)
        .then(({ data }) => {
    ...
    if (shouldBeRunAtStart || runDocker) {
      ...
      axiosClient
        .get(
          `${
            lang === undefined ? '/' : '/' + lang + '/'
          }compiler/@${username}/${project}/start-docker-session`,
        )
        .then((res) => {
          console.log(res);
          if (res.status === 204) {
                        ...
                        <div className="compiler-part">
                          ...
                          >
                            ...
                            <>
                              ...
                              <CompilerConsole
                                sendRequestTerminal={sendRequestTerminal}
                                taskIsLoading={taskIsLoading}
