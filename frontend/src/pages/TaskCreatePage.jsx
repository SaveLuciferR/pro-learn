...
const TaskCreatePage = ({ type }) => {
  ...
  const updateTemplateData = () => {
    axiosClient
      .get(`${lang === undefined ? '/' : '/' + lang + '/'}@${username}/template-list?type=all`)
      .then(({ data }) => {
  ...
  useEffect(() => {
    if (type === 'edit') {
      axiosClient
        .get(`/@${username}/creation/edit-task/${slug}`)
        .then(({ data }) => {
      ...
      {!successToGetTask ? (
        ...
        <div className="profile-section">
          ...
          <div className="profile-section-main">
            <h1 className="profile-settings-title big text-center-create">
              {type === 'edit' ? '_Изменить задачу' : '_Создать задачу'}
            </h1>
            <div className="course-create-header">
              <div className="course-create-second_header task-second_header">
                    ...
                    <ul className="profile-difficulty-range inline">
                      {difficultyAmount.map((item, i) => (
                        <li
                          ...
                          className={`hover profile-difficulty-range-item ${item ? 'active' : ''}`}
                        ></li>
                      ))}
                    </ul>
                  </div>
                </div>
                <div className="course-create-down task">
                  <div className="course-create-down-dropdown">
                    <DropdownCheckbox
                      title={'Категория'}
                    ...
                    <DropdownCheckbox
                      ...
                      setCheckContent={setCheckContent2}
                    />
                  </div>

                  <div className="course-create-down-buttons">
                    <div>
                      <Link
                        ...
                        to={`${
                          lang === undefined ? '/' : '/' + lang + '/'
                        }profile/${username}/project/${slugBindProject}`}
                        target={'_blank'}
                      >
                        Перейти к проекту
                      </Link>
                      <button
                        onClick={() => setOpenModalTaskProject(true)}
                        className={'btn underline-hover'}
                      >
                        Привязать проект
                      </button>
                    </div>
                    <div>
                      <Link
                        ...
                        to={`${
                          lang === undefined ? '/' : '/' + lang + '/'
                        }profile/${username}/project/${slugBindProject}`} // TODO страница с шаблоном
                        target={'_blank'}
                      >
                        Перейти к шаблону
                      </Link>
                      <button
                        onClick={() => setOpenModalTaskTemplate(true)}
                        className={'btn underline-hover'}
                      >
                        Привязать шаблон
                      </button>
                ...
                <div>
                  <Switch
                    text={'Задача только для курса'}
                    isToggle={currentTask.for_course}
