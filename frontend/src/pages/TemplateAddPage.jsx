...
const TemplateAddPage = ({type = 'create'}) => {
    ...
    // if (!userAuth) navigate('/user/login');
    ...
    const [forProject, setForProject] = useState(true);
    ...
    const handleFileDelete = (e, path, fileName) => {
        ...
        path += fileName;
        axiosClient.post(`/@${username}/project-add/delete-file`, {path, type, slug, template: true})
            .then((data) => {
    ...
    useEffect(() => {
        if (type === 'edit') {
            axiosClient.get(`/@${username}/template-edit/${slug}`)
                .then((res) => {
                    console.log(res.data)
                    setInfoTemplate(res.data.template.main);
                    setNeedUpdateTable(true);
                    // setNameProject(res.data.project.title)
                    // setDescProject(res.data.project.description)
                    setForProject(res.data.template.for_project);
                    setPrivacyProject(res.data.template.private);
                })
                .catch((error) => {
                    console.log(error);
                })
            ...
            Object.keys(languages).map((item, i) => {
                info[languages[item].id] = {};
                info[languages[item].id].name = '';
                info[languages[item].id].desc = '';
            });
    ...
    const onClickSaveNewProject = () => {
        ...
        if (type === 'edit') {
            axiosClient.post(`/@${username}/template-save-edit/${slug}`, {
                username,
                infoTemplate,
                forProject,
                // nameProject,
                ...
                .then((res) => {
                    console.log(res);
                        navigate(`${lang === undefined ? '/' : '/' + lang + '/'}profile/${username}/template/${slug}`)
                })
                .catch((error) => {
                    console.log(error);
                })
        } else {
            axiosClient.post(`/@${username}/template-save-new`, {
                // mainFolderProject,
                username,
                infoTemplate,
                forProject,
                ...
                .then(({data}) => {
                    // console.log(data);
                    navigate(`${lang === undefined ? '/' : '/' + lang + '/'}profile/${username}/template/${data.slug}`);
                });
        }
    }
        ...
        <div className="project">
            <div className="project__container">
                <div className="project__inner">
                    <h1 className="markdown-h1 project__add-title">{type === 'edit' ? 'Изменить шаблон' : 'Создать шаблон'}</h1>

                    <div className="project__add-body">

                        <div className="project__add-body_name">
                            <Switch text={"Используется для проектов"} isToggle={forProject}
                                    onToggle={() => setForProject(!forProject)}/>
                        ...
                        {Object.keys(infoTemplate).length === 0 ?
                            <LoadingElement/>
                            :
                            <>
                                ...
                                {
                                    Object.keys(languages).map((item, i) =>
                                        <TemplateAddTab key={i} data={infoTemplate}
                                                        setData={setInfoTemplate}
                                                        type={type}
                                                        currentLang={languages[item].id}
                        ...
                        {/*          placeholder="Описание шаблона" value={descProject}*/}
                        ...
                        <div className="project__add-buttons">
                            ...
                            <button className="btn primary big" type="button"
                                    onClick={() => onClickSaveNewProject()}>Сохранить шаблон
                            </button>
                        </div>

                        <div className="project__add-body_desc-files">
                            Выберите папку с шаблоном
                            <br/>
