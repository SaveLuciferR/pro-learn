...
const CourseCreatePage = ({ type = 'create' }) => {
  ...
  }, [userAuth, user, username, navigate]);


  useEffect(() => {
    if (type === 'edit') {
      axiosClient.get(`@${username}/creation/edit-course/${slug}`).then(({ data }) => {
    ...
    axiosClient
      .get(
        `${lang === undefined ? '/' : '/' + lang + '/'
        }@${username}/creation/course/category-lang-prog`,
      )
      .then(({ data }) => {
        console.log(data);
  ...
  }, [username, slug, lang, type, dispatch]);
  ...
  useEffect(() => {
    if (uploadIcon) {
      axiosClient
        .post(
          `@${username}/creation/course/save-icon`,
          { icon: uploadIcon },
    ...
    if (successToGetCourse) {
      dispatch(
        setCurrentCourseMainInfo({
          difficulty,
          icon,
          category_prog: categoryLang,
          lang_prog: langProg,
        }),
      );
    }
  }, [langProg, difficulty, icon, categoryLang, dispatch, successToGetCourse]);
        ...
        <div className="profile-section">
          ...
          <div className="profile-section-main">
            <h1 className="profile-settings-title big text-center-create">
              {type === 'edit' ? '_Изменить курс' : '_Создать курс'}
            </h1>
            ...
            {isProfileSidebar ? <ProfilePopUpSidebar /> : <></>}
            <div className="course-create-header">
              <label className="input-file course-create-icon">
                <div className="sidebar_profile-create_item">
                  <input
                    height={'100%'}
                    hidden={true}
                    onChange={(e) => setUploadIcon(e.target.files)}
                    type="file"
                    accept={'image/*'}
                    name="project"
                  />
              ...
              <div className="course-create-second_header">
                ...
                <div className="course-create-down">
                  <div
                    className={`dropdown ${dropdownActive1 ? 'active' : ''}`}
                  ...
                  <div
                    className={`dropdown ${dropdownActive2 ? 'active' : ''}`}
                    style={{ maxWidth: '300px', width: '100%' }}
